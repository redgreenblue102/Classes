...
def display_menu():
    ...
    print("min  - View movies less than a number of minutes")
...
def display_movies_by_minutes():
    mins = int(input("Maximum number of minutes: "))
    display_movies(db.get_movies_by_minutes(mins),f"LESS THAN {mins} MINUTES")
...
def delete_movie(): #update code here
    movie_id = int(input("Movie ID: "))
    mname=db.get_movie(movie_id).name 
    print("Are you sure you want to delete " + mname + "? (y/n): ")
    d=input()
    if(d=='y'):
        db.delete_movie(movie_id)
        print(f"'{mname}' was deleted from database.\n")
        
def main(): #update for min
    db.connect()
    display_title()
    display_categories()
    while True:        
